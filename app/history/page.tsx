...
export default function HistoryPage() {
  ...
  const stats = useMemo(() => {
	...
	const bestTrade = Math.max(...matchHistory.map((m) => m.realized_pnl));
	const worstTrade = Math.min(...matchHistory.map((m) => m.realized_pnl));
		  ...
		  <div className="flex flex-col gap-2.5">
			{filteredMatches.length === 0 ? (
			  ...
			  filteredMatches.map((match) => (
				<div
				  key={match.id}
				  className={`group relative rounded-[10px] border border-white/[.07] bg-[#0f131b] p-4 transition-all duration-200 hover:border-white/[.14] hover:-translate-y-[1px] cursor-pointer ${getResultGlow(match.result)}`}
				>
