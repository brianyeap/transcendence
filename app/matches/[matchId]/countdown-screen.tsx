...
export function CountdownScreen({
  ...
  const t = useTranslations("CountdownScreen");
      ...
      <div className="w-full max-w-lg rounded-xl border border-white/[.07] bg-[#0f131b] p-6 sm:p-7">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-3.5 text-[21px] font-bold tracking-[-.01em]">
            {t("opensIn", { symbol: match.symbol })}
          </h1>

          <p className="mt-2 font-mono text-[64px] font-semibold leading-none tracking-[-.03em] tabular-nums text-[#eef2f8]">
            {starting
              ? "—"
              : remaining < BARE_SECONDS_UNDER
                ? remaining
                : fmtClock(remaining)}
          </p>

          <p className="mt-3 text-[13px] text-[#9aa6b6]">
            {starting ? t("starting") : t("getReady")}
          </p>
        </div>
